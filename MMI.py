...
sources = [mp.EigenModeSource(src= mp.GaussianSource(freq, fwidth=fwidth), center= mp.Vector3(-end_of_input-0.25, -wg_center_y, 0), size=mp.Vector3(0, w_Input+wvl, 2.8), direction=mp.X)]
...
incident_mode_alpha = sim.get_eigenmode_coefficients(incident_mode,[1], direction=mp.Z).alpha[0,:,0]

incident_flux_data = sim.get_flux_data(incident_mode)

# Shows the electric field in the z direction
ez_data_center = sim.get_array(center=mp.Vector3(), size=mp.Vector3(l_cell, w_cell, 0), component=mp.Ez)
ez_data_top = sim.get_array(center=mp.Vector3(0,0,h_cell-w_Absorber), size=mp.Vector3(l_cell, w_cell, 0), component=mp.Ez)
ez_data_bottom = sim.get_array(center=mp.Vector3(0,0,-h_cell+w_Absorber), size=mp.Vector3(l_cell, w_cell, 0), component=mp.Ez)
if mp.am_master():
    plt.imsave("StraightEzFieldsCenter.png", ez_data_center.transpose(), cmap='RdBu')
    plt.imsave("StraightEzFieldsTop.png", ez_data_top.transpose(), cmap='RdBu')
    plt.imsave("StraightEzFieldsBottom.png", ez_data_bottom.transpose(), cmap='RdBu')
...
outputLoAlpha = sim.get_eigenmode_coefficients(output_lo_mode,[1], direction=mp.Z).alpha[0,:,0]
outputHiAlpha = sim.get_eigenmode_coefficients(output_hi_mode,[1], direction=mp.Z).alpha[0,:,0]
inputLoAlpha = sim.get_eigenmode_coefficients(input_mode,[1], direction=mp.Z).alpha[0,:,1]
inputHiAlpha = sim.get_eigenmode_coefficients(input_hi_mode,[1], direction=mp.Z).alpha[0,:,1]
...
ez_data_bottom = sim.get_array(center=mp.Vector3(0,0,-h_cell+w_Absorber), size=mp.Vector3(l_cell, w_cell, 0), component=mp.Ez)
if mp.am_master():
    plt.imsave("MMIEzFieldsCenter.png", ez_data_center.transpose(), cmap='RdBu')
    plt.imsave("MMIEzFieldsTop.png", ez_data_top.transpose(), cmap='RdBu')
    plt.imsave("MMIEzFieldsBottom.png", ez_data_bottom.transpose(), cmap='RdBu')
    ...
    fig.set_xlabel('Wavelength (um)')
    ax1.set_title("Low Transmission")
    ax2.set_title("High Transmission")
    ax1.plot(wvls[:],trans_Lo, color='blue')
    ax2.plot(wvls[:],trans_Hi, color='red')
    fig.savefig("Tranmission.png")

    # Plotting the reflectance
    fig, (ax1, ax2) = plt.subplots(1,2, sharex='all')
    fig.set_xlabel('Wavelength (um)')
    ax1.set_title("Low Reflectance")
    ax2.set_title("High Reflectance")
    ax1.plot(wvls[:],refl_Lo, color='blue')
    ax2.plot(wvls[:],refl_Hi, color='red')
    fig.savefig("Reflectance.png")
